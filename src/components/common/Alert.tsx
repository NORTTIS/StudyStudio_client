...
export function Alert({ variant = "info", title, message, onClose, className }: AlertProps) {
    ...
    return (
        <div className={twMerge("flex gap-3 rounded-lg border px-4 py-3 text-sm", variantStyles[variant], className)}>
...
export function ErrorMessage({ message, className }: ErrorMessageProps) {
    return <p className={twMerge("text-red-600 text-xs", className)}>{message}</p>;
}

/**
 * SuccessMessage - Compact inline success display
 */
...
export function SuccessMessage({ message, className }: SuccessMessageProps) {
    return <p className={twMerge("text-green-700 text-xs", className)}>{message}</p>;
}
