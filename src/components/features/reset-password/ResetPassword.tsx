...
export function ResetPassword({ token }: Props) {
  ...
  useEffect(() => {
    const verifyToken = async (tokenToVerify: string) => {
      ...
      const result = await verifyResetToken(tokenToVerify, locale);

      if (result.status === "success") {
        setIsValidToken(true);
        setResetToken(tokenToVerify);

        if (typeof window !== "undefined") {
          sessionStorage.setItem(resetTokenStorageKey, tokenToVerify);
        }
      } else {
    ...
    if (typeof window !== "undefined") {
      const storedToken = sessionStorage.getItem(resetTokenStorageKey);
      if (storedToken) {
  ...
  const onSubmit = async (data: ResetPasswordFormData) => {
    ...
    const result = await apiPost(
      ...
      payload,
      locale,
      true // skipAuth - reset password doesn't require authentication
    );

    if (result.status === "error") {
      setError(result.message || t("invalidTokenMessage"));
      return;
    }
