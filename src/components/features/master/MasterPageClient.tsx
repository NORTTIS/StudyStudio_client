...
export default function MasterPageClient({ initialUserProfile, initialStudios }: MasterPageClientProps) {
    ...
    const [filteredStudios, setFilteredStudios] = useState<StudioUI[]>(initialStudios);
    ...
    const handleCreateStudio = async (data: { name: string; description: string; type: string }) => {
        if (studios.length >= 3) {
            toast({
                description: t("modal.limitReached") || "Báº¡n Ä‘Ă£ Ä‘áº¡t giá»›i háº¡n 3 studio",
                variant: "destructive"
            });
            return;
        }
