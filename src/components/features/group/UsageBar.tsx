export function UsageBar({ current, max }: { current: number; max: number }) {
    ...
    return (
        <div
            className={`mt-4 w-full rounded-2xl border px-6 py-4 ${
                isFull ? "border-orange-200 bg-orange-50" : "border-[#E5E5E5] bg-white"
            }`}>
            {isFull ? (
                <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                        ...
                        <p className="mt-1 text-sm text-[#6F6B99]">
                            <span className="font-semibold text-orange-600">Nâng cấp lên gói đăng ký</span> để có thể
                            tạo lên tới 10 không gian nhóm.
                        </p>
                ...
                <>
                    <p className="text-xs text-[#6F6B99]">
                        <span className="font-semibold text-[#261E33]">{current}</span> / {max} không gian nhóm đang
                        được sử dụng.
                    </p>
