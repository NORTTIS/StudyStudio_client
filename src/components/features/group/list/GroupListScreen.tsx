...
const readText = async (res: Response) => { try { return await res.text(); } catch { return ""; } };

function parseMaybeJson(raw: string) {
    const text = (raw ?? "").toString().trim();
    if (!text) return { json: null as any, text: "" };
    try { return { json: JSON.parse(text.replace(/^\uFEFF/, "")), text }; }
...
function getApiBase() { return String(process.env.NEXT_PUBLIC_API_BASE_URL || "").replace(/\/+$/, ""); }
...
async function apiReorderGroupTaskStatus(args: { groupId: string; statusId: string; prevStatusId: string | null; nextStatusId: string | null }) {
    const apiBase = getApiBase(); const token = getAccessTokenOrNull();
    if (!apiBase) throw new Error("Thiếu NEXT_PUBLIC_API_BASE_URL.");
    ...
    const res = await fetch(`${apiBase}/GroupTaskStatus/${encodeURIComponent(args.groupId)}/reorder`, {
        ...
        headers: { Accept: "text/plain, application/json", "Content-Type": "application/json", ...(token ? { Authorization: `Bearer ${token}` } : {}) },
        body: JSON.stringify({ statusId: args.statusId, prevStatusId: args.prevStatusId, nextStatusId: args.nextStatusId })
    });
    const raw = await readText(res); const { json } = parseMaybeJson(raw);
...
async function apiCreateGroupTaskStatus(args: { groupId: string; statusName: string; position: number }) {
    ...
    if (!apiBase) throw new Error("Thiếu NEXT_PUBLIC_API_BASE_URL.");
    if (!isUuidLike(args.groupId)) throw new Error("groupId không hợp lệ.");
    const payload = { statusName: String(args.statusName ?? "").trim(), position: Math.max(0, Math.trunc(args.position)) };
...
async function apiCreateTask(args: { groupId: string; groupStatusId: string; taskName: string }) {
    ...
    const res = await fetch(`${apiBase}/Task`, {
        method: "POST", credentials: "include",
        headers: { Accept: "text/plain, application/json", "Content-Type": "application/json", ...(token ? { Authorization: `Bearer ${token}` } : {}) },
        body: JSON.stringify({ groupId: args.groupId, groupStatusId: args.groupStatusId, taskName: String(args.taskName ?? "").trim() })
    });
    const raw = await readText(res); const { json } = parseMaybeJson(raw);
    if (!res.ok || !(json ? okByJsonStatus(json) : true)) throw new Error(extractApiMessage(raw, json));
...
async function apiRenameGroupTaskStatus(args: { groupId: string; statusId: string; statusName: string; position: number }) {
    ...
    const res = await fetch(`${apiBase}/GroupTaskStatus/${encodeURIComponent(args.groupId)}/${encodeURIComponent(args.statusId)}`, {
        method: "PUT", credentials: "include",
        headers: { Accept: "text/plain, application/json", "Content-Type": "application/json", ...(token ? { Authorization: `Bearer ${token}` } : {}) },
        body: JSON.stringify({ position: Math.max(0, Math.trunc(args.position)), statusName: String(args.statusName ?? "").trim() })
    });
    const raw = await readText(res); const { json } = parseMaybeJson(raw);
    if (!res.ok || !(json ? okByJsonStatus(json) : true)) throw new Error(extractApiMessage(raw, json));
    return true;
...
async function apiDeleteGroupTaskStatus(args: { groupId: string; statusId: string }) {
    ...
    if (!isUuidLike(args.groupId)) throw new Error("groupId không hợp lệ.");
    if (!isUuidLike(args.statusId)) throw new Error("statusId không hợp lệ.");
    const res = await fetch(`${apiBase}/GroupTaskStatus/${encodeURIComponent(args.statusId)}/group/${encodeURIComponent(args.groupId)}`, {
...
function ConfirmModal({ open, title, description, confirmLabel = "Xác nhận", cancelLabel = "Hủy", onConfirm, onCancel }: {
        ...
        <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4"
            ...
            <div className="w-full max-w-sm rounded-2xl bg-white p-6 shadow-2xl" onPointerDown={(e) => e.stopPropagation()}>
                ...
                <div className="mt-6 flex items-center justify-end gap-3">
                    <button type="button" onClick={onCancel} className="rounded-xl border border-zinc-200 bg-white px-4 py-2 text-sm font-semibold text-zinc-700 hover:bg-zinc-100 transition">{cancelLabel}</button>
                    <button type="button" onClick={onConfirm} className="rounded-xl bg-rose-600 px-4 py-2 text-sm font-semibold text-white hover:bg-rose-700 transition">{confirmLabel}</button>
                </div>
            </div>
        </div>,
        document.body
...
function PortalDropdown({ open, onClose, anchorRef, children }: {
    open: boolean; onClose: () => void; anchorRef: React.RefObject<HTMLElement>; children: React.ReactNode;
}) {
    ...
    const syncPos = React.useCallback(() => {
        ...
        const r = a.getBoundingClientRect(); const width = 208;
        setPos({ top: r.bottom + 8, left: Math.max(8, r.right - width), width });
    }, [anchorRef]);

    React.useEffect(() => {
        if (!open) return; syncPos();
        const onScroll = () => syncPos(); const onResize = () => syncPos();
    ...
    React.useEffect(() => {
        ...
        const onPD = (e: PointerEvent) => {
            ...
            if (menuRef.current?.contains(t)) return; if (anchorRef.current?.contains(t)) return;
            onClose();
        };
        const onKD = (e: KeyboardEvent) => { if (e.key === "Escape") onClose(); };
        window.addEventListener("pointerdown", onPD, true); window.addEventListener("keydown", onKD);
        return () => { window.removeEventListener("pointerdown", onPD, true); window.removeEventListener("keydown", onKD); };
...
function AddTaskInline({ disabled, onSubmit }: { disabled: boolean; onSubmit: (title: string) => Promise<void> }) {
    ...
    React.useEffect(() => { if (open) { setError(null); setTimeout(() => inputRef.current?.focus(), 0); } }, [open]);

    const close = () => { setOpen(false); setTitle(""); setError(null); };
    const submit = async () => {
        const trimmed = title.trim();
        if (!trimmed) { setError("Vui lòng nhập tên công việc."); inputRef.current?.focus(); return; }
        try { setError(null); await onSubmit(trimmed); close(); }
        catch (e: any) { setError(e?.message ?? "Tạo công việc thất bại"); inputRef.current?.focus(); }
    };

    if (!open) return (
        <button type="button" onClick={() => setOpen(true)} disabled={disabled}
            className={cn("flex w-full items-center gap-2 px-4 py-3 text-sm font-medium text-zinc-500 hover:bg-zinc-50 hover:text-zinc-700 transition border-t border-zinc-100",
        ...
        <div className="border-t border-zinc-100 px-4 py-3">
            <div className="flex items-center gap-2">
                <input ref={inputRef} value={title} onChange={(e) => setTitle(e.target.value)}
                    onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); void submit(); } if (e.key === "Escape") { e.preventDefault(); close(); } }}
                    disabled={disabled} placeholder="Nhập tên công việc..."
                ...
                <button type="button" onClick={() => void submit()} disabled={disabled}
                    className={cn("rounded-lg bg-indigo-600 px-3 py-2 text-sm font-semibold text-white hover:bg-indigo-700 transition", disabled && "opacity-60 pointer-events-none")}>
                    Lưu
                </button>
...
function AddColumnInline({ isSubmitting, onSubmit }: { isSubmitting: boolean; onSubmit: (title: string) => Promise<void> }) {
    ...
    const submit = async () => {
        const trimmed = title.trim();
        if (!trimmed) { setError("Vui lòng nhập tên trạng thái."); inputRef.current?.focus(); return; }
        try { setError(null); await onSubmit(trimmed); close(); }
        catch (e: any) { setError(e?.message ?? "Tạo trạng thái thất bại"); inputRef.current?.focus(); }
    };
    ...
    return (
        <div className="rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm">
            <input ref={inputRef} value={title} onChange={(e) => setTitle(e.target.value)}
                onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); void submit(); } if (e.key === "Escape") { e.preventDefault(); close(); } }}
                disabled={isSubmitting} placeholder="Nhập tên trạng thái..."
            ...
            <div className="mt-3 flex items-center gap-2">
                <button type="button" onClick={() => void submit()} disabled={isSubmitting}
                    className={cn("rounded-xl px-3 py-2 text-sm font-semibold text-white bg-indigo-600 hover:bg-indigo-700 transition", isSubmitting && "opacity-60 pointer-events-none")}>
                    Thêm trạng thái
                </button>
                <button type="button" onClick={close} disabled={isSubmitting}
                    className={cn("grid h-9 w-9 place-items-center rounded-xl border border-zinc-200 bg-white text-zinc-700 hover:bg-zinc-100 transition", isSubmitting && "opacity-60 pointer-events-none")}>
                    <X className="h-4 w-4" />
                </button>
            </div>
        </div>
...
function SortableTaskRow({ task, columnId, onStartEdit, onDelete }: {
    task: Task; columnId: ColumnId;
    onStartEdit: () => void; onDelete: () => void;
}) {
    const { attributes, listeners, setNodeRef, setActivatorNodeRef, transform, transition, isDragging } = useSortable({
        id: task.id, data: { type: "task", columnId }
    });
        ...
        <div ref={setNodeRef} style={style}
            ...
            <div className="col-span-1 flex justify-end">
                ...
                <PortalDropdown open={openMenu} onClose={() => setOpenMenu(false)} anchorRef={btnRef as any}>
                    <MenuItem icon={<Pencil className="h-4 w-4" />} label="Chỉnh sửa tên" onClick={() => { setOpenMenu(false); onStartEdit(); }} />
                    <MenuItem icon={<Trash2 className="h-4 w-4" />} label="Xóa" danger onClick={() => { setOpenMenu(false); onDelete(); }} />
                </PortalDropdown>
            </div>
        </div>
    );
...
function EditTaskRow({ task, draft, onDraftChange, onCommit, onCancel }: {
    task: Task; draft: string; onDraftChange: (v: string) => void; onCommit: () => void; onCancel: () => void;
}) {
    const inputRef = React.useRef<HTMLInputElement | null>(null);
    ...
    return (
        <div className="grid grid-cols-12 items-center gap-3 border-t border-zinc-100 bg-indigo-50/40 px-4 py-3">
            <div className="col-span-6 flex items-center gap-3 min-w-0">
                ...
                <input ref={inputRef} value={draft} onChange={(e) => onDraftChange(e.target.value)}
                    onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); onCommit(); } if (e.key === "Escape") { e.preventDefault(); onCancel(); } }}
            ...
            <div className="col-span-6 flex items-center justify-end gap-2">
                <button type="button" onClick={onCommit} className="rounded-lg bg-indigo-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-indigo-700">Lưu</button>
                <button type="button" onClick={onCancel} className="grid h-7 w-7 place-items-center rounded-lg border border-zinc-200 bg-white text-zinc-600 hover:bg-zinc-100"><X className="h-3.5 w-3.5" /></button>
...
function ColumnDropZone({ colId, isOver, setNodeRef, children }: {
    colId: ColumnId; isOver: boolean; setNodeRef: (node: HTMLElement | null) => void; children: React.ReactNode;
}) {
    return (
...
function SortableSection({ col, tasks, taskIds, groupId, creatingTask, onCreateTask, onRename, onDeleteColumn,
    taskEditState, onTaskStartEdit, onTaskCancelEdit, onTaskDraftChange, onTaskCommitEdit, onDeleteTask,
    isColumnEditing, columnDraft, columnError, onColumnDraftChange, onColumnCommit, onColumnCancel
}: {
    col: Column; tasks: Task[]; taskIds: string[]; groupId: string; creatingTask: boolean;
    onCreateTask: (colId: ColumnId, title: string) => Promise<void>;
    onRename: (colId: ColumnId) => void; onDeleteColumn: (colId: ColumnId) => void;
    ...
    onTaskCancelEdit: () => void; onTaskDraftChange: (v: string) => void; onTaskCommitEdit: () => void;
    onDeleteTask: (taskId: string, columnId: ColumnId) => void;
    isColumnEditing: boolean; columnDraft: string; columnError: string | null;
    onColumnDraftChange: (v: string) => void; onColumnCommit: () => void; onColumnCancel: () => void;
}) {
    const { attributes, listeners, setNodeRef: setSortableRef, setActivatorNodeRef, transform, transition, isDragging } = useSortable({
        id: col.id, data: { type: "column" }
    });

    const dropId = `drop:${col.id}`;
    const { setNodeRef: setDropRef, isOver } = useDroppable({ id: dropId, data: { type: "column-drop", columnId: col.id } });
    ...
    React.useEffect(() => { if (isColumnEditing) setTimeout(() => colInputRef.current?.focus(), 0); }, [isColumnEditing]);

    return (
        <div ref={setSortableRef} style={style} className="overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-sm">
            {/* Header */}
            <div className="flex items-center justify-between gap-3 px-4 py-3">
                <div className="flex flex-1 items-center gap-3 min-w-0">
                    <button type="button" onClick={() => setOpen((v) => !v)} className="shrink-0 text-zinc-400 hover:text-zinc-600 transition">
                    ...
                    <div className="min-w-0 flex-1">
                        {!isColumnEditing ? (
                            ...
                            <div className="space-y-0.5">
                                <input ref={colInputRef} value={columnDraft}
                                    onChange={(e) => onColumnDraftChange(e.target.value)}
                                    onPointerDownCapture={(e) => e.stopPropagation()}
                                    onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); onColumnCommit(); } if (e.key === "Escape") { e.preventDefault(); onColumnCancel(); } }}
                                    onBlur={() => setTimeout(() => onColumnCommit(), 0)}
                                    className={cn("h-8 w-full rounded-lg border bg-white px-2 text-sm font-bold text-zinc-900 outline-none",
                                        columnError ? "border-rose-300 focus:ring-2 focus:ring-rose-100" : "border-zinc-200 focus:border-indigo-300 focus:ring-2 focus:ring-indigo-200",
                                        "select-text")} style={{ maxWidth: 220 }} />
                ...
                <div className="flex shrink-0 items-center gap-2">
                    ...
                    <button ref={colMenuBtnRef} type="button"
                        onPointerDown={(e) => { e.preventDefault(); e.stopPropagation(); setOpenColMenu((v) => !v); }}
                        onClick={(e) => { e.preventDefault(); e.stopPropagation(); }}
                        className="grid h-8 w-8 place-items-center rounded-xl text-zinc-500 hover:bg-zinc-100" aria-label="Column menu">
                        <MoreHorizontal className="h-4 w-4" />
                    </button>
                    <PortalDropdown open={openColMenu} onClose={() => setOpenColMenu(false)} anchorRef={colMenuBtnRef as any}>
                        <MenuItem icon={<Pencil className="h-4 w-4" />} label="Chỉnh sửa" onClick={() => { setOpenColMenu(false); onRename(col.id); }} />
                        <MenuItem icon={<Trash2 className="h-4 w-4" />} label="Xóa" danger onClick={() => { setOpenColMenu(false); onDeleteColumn(col.id); }} />
                    </PortalDropdown>
                </div>
            </div>
                    ...
                    <ColumnDropZone colId={col.id} isOver={isOver} setNodeRef={setDropRef}>
                        <SortableContext items={taskIds} strategy={verticalListSortingStrategy}>
                            {tasks.map((task) => {
                                ...
                                if (isEditing) {
                                    return (
                                        <EditTaskRow key={task.id} task={task}
                                ...
                                return (
                                    <SortableTaskRow key={task.id} task={task} columnId={col.id}
...
export function GroupListScreen() {
    ...
    const [editingColumn, setEditingColumn] = React.useState<{ id: string | null; draft: string; error: string | null }>({ id: null, draft: "", error: null });
    const [editingTask, setEditingTask] = React.useState<{ taskId: string | null; columnId: string | null; draft: string }>({ taskId: null, columnId: null, draft: "" });
    const [confirmModal, setConfirmModal] = React.useState<{ open: boolean; columnId: ColumnId | null; columnTitle: string }>({ open: false, columnId: null, columnTitle: "" });
    ...
    const syncColumnsFromDetail = React.useCallback((detail: GroupDetailResponse | undefined) => {
        const statuses = (detail?.taskStatuses ?? [])
            ...
            .map((s) => ({ id: String(s.statusId), title: String(s.statusName ?? ""), position: Number.isFinite(s.position) ? (s.position as number) : 0 }))
            .sort((a, b) => a.position - b.position);
        setColumns(statuses);
        setBoard((prev) => { const next: Record<string, Task[]> = {}; for (const c of statuses) next[c.id] = prev[c.id] ?? []; return next; });
    }, []);

    const refresh = React.useCallback(async () => {
        if (!groupId) { setLoading(false); setLoadError("Thiếu groupId trong route."); return; }
        if (!isUuidLike(groupId)) { setLoading(false); setLoadError("groupId không hợp lệ."); return; }
        if (!getApiBase()) { setLoading(false); setLoadError("Thiếu NEXT_PUBLIC_API_BASE_URL."); return; }
        setLoading(true); setLoadError(null);
        try { const detail = await apiGetGroupDetail(groupId); syncColumnsFromDetail(detail?.data); }
    ...
    const activeTask = React.useMemo(() => activeTaskId ? findTask(board, columns, activeTaskId) : null, [activeTaskId, board, columns]);
    const activeColumn = React.useMemo(() => activeColumnId ? columns.find((c) => c.id === activeColumnId) ?? null : null, [activeColumnId, columns]);

    // ── Column CRUD ──────────────────────────────────────────────────────────
    const submitAddColumn = async (title: string) => {
        if (!groupId || !isUuidLike(groupId)) throw new Error("groupId không hợp lệ.");
        if (columns.some((c) => c.title.trim().toLowerCase() === title.trim().toLowerCase())) throw new Error("Tên trạng thái đã tồn tại.");
        const base = detectPositionBase(columns) as 0 | 1;
        setCreatingColumn(true);
        try { await apiCreateGroupTaskStatus({ groupId, statusName: title, position: nextPositionForCreate(columns, base) }); await refresh(); }
    ...
    const commitEditColumn = async () => {
        if (!groupId) return; const id = editingColumn.id; if (!id) return;
        const col = columns.find((c) => c.id === id); if (!col) return;
        const next = editingColumn.draft.trim();
        if (!next) { setEditingColumn((p) => ({ ...p, error: "Vui lòng nhập tên trạng thái." })); return; }
        if (columns.some((c) => c.id !== id && c.title.trim().toLowerCase() === next.toLowerCase())) {
            setEditingColumn((p) => ({ ...p, error: "Tên trạng thái đã tồn tại." })); return;
        }
        const prevTitle = col.title;
        setColumns((prev) => prev.map((c) => (c.id === id ? { ...c, title: next } : c)));
        try { await apiRenameGroupTaskStatus({ groupId, statusId: id, statusName: next, position: col.position }); cancelEditColumn(); await refresh(); }
        catch (e: any) { setColumns((prev) => prev.map((c) => (c.id === id ? { ...c, title: prevTitle } : c))); setEditingColumn((p) => ({ ...p, error: e?.message ?? "Đã xảy ra lỗi" })); }
    };
    ...
    const handleConfirmDeleteColumn = async () => {
        ...
        const prevCols = columns; const prevBoard = board;
        const base = detectPositionBase(columns) as 0 | 1;
        setColumns(assignPositions(columns.filter((c) => c.id !== columnId), base));
        setBoard((prev) => { const next = { ...prev }; delete next[columnId]; return next; });
        try { await apiDeleteGroupTaskStatus({ groupId, statusId: columnId }); await refresh(); }
        catch (e: any) { setColumns(prevCols); setBoard(prevBoard); alert(e?.message ?? "Đã xảy ra lỗi"); }
    };
    ...
    const onCreateTask = async (columnId: ColumnId, title: string) => {
        ...
        try {
            ...
            const next: Task = { id, title: created?.data?.taskTitle ?? title, statusDot: "green", tagLeft: "TASK", tagRight: "SS", due: created?.data?.dueDate ?? "" };
            setBoard((prev) => ({ ...prev, [columnId]: [next, ...(prev[columnId] ?? [])] }));
        } finally { setCreatingTask(false); }
    };

    const onTaskStartEdit = (taskId: string, columnId: ColumnId, title: string) => setEditingTask({ taskId, columnId, draft: title });
    const onTaskCancelEdit = () => setEditingTask({ taskId: null, columnId: null, draft: "" });
    const onTaskCommitEdit = () => {
        const { taskId, columnId, draft } = editingTask; if (!taskId || !columnId) return;
        const next = draft.trim(); if (!next) { onTaskCancelEdit(); return; }
        setBoard((prev) => ({ ...prev, [columnId]: (prev[columnId] ?? []).map((t) => (t.id === taskId ? { ...t, title: next } : t)) }));
        onTaskCancelEdit();
    };
    ...
    const collisionDetection: CollisionDetection = React.useCallback((args) => {
        ...
        if (activeType === "column") return closestCenter({ ...args, droppableContainers: filterDroppablesByType(args.droppableContainers, ["column"]) });
        return closestCorners({ ...args, droppableContainers: filterDroppablesByType(args.droppableContainers, ["task", "column-drop"]) });
    }, []);
    ...
    const handleDragEnd = (e: DragEndEvent) => {
        ...
        setActiveTaskId(null); setActiveColumnId(null);
        if (!overRaw) return;

        if (activeType === "task") {
            const activeId = String(e.active.id);
            const overKey = overRaw.startsWith("drop:") ? overRaw.replace("drop:", "") : overRaw;
            const fromCol = findColumnOfTask(board, columns, activeId); if (!fromCol) return;
            let toCol: ColumnId | null = columns.some((c) => c.id === overKey) ? overKey : findColumnOfTask(board, columns, overKey);
            if (!toCol) return;

            setBoard((prev) => {
                const fromTasks = [...(prev[fromCol] ?? [])];
                const toTasks = fromCol === toCol ? fromTasks : [...(prev[toCol!] ?? [])];
                const fromIdx = fromTasks.findIndex((t) => t.id === activeId); if (fromIdx === -1) return prev;
                if (fromCol === toCol) {
                ...
                if (overIsTask) { const idx = toTasks.findIndex((t) => t.id === overKey); toTasks.splice(Math.max(0, idx), 0, moving); }
        ...
        if (activeType === "column") {
            ...
            if (!columns.some((c) => c.id === overColId)) { const m = findColumnOfTask(board, columns, overColId); if (m) overColId = m; }
    ...
    if (loading) return (
        <div className="min-h-screen bg-zinc-50">
            <Container>
                <div className="mt-6 rounded-2xl border border-zinc-200 bg-white px-4 py-4 text-sm text-zinc-700">Đang tải dữ liệu…</div>
            </Container>
        </div>
    );
    ...
    if (loadError) return (
        <div className="min-h-screen bg-zinc-50">
            <Container>
                ...
                <div className="mt-3">
                    <button type="button" onClick={() => void refresh()} className="rounded-xl border border-zinc-200 bg-white px-3 py-2 text-sm font-semibold text-zinc-900 hover:bg-zinc-100">Tải lại</button>
                </div>
            </Container>
        </div>
    );
    ...
    const sectionProps = (col: Column) => ({
        col, tasks: board[col.id] ?? [], taskIds: taskIdsByCol[col.id] ?? [],
        groupId, creatingTask,
        onCreateTask, onRename: startEditColumn, onDeleteColumn,
        taskEditState: editingTask, onTaskStartEdit, onTaskCancelEdit,
        onTaskDraftChange: (v: string) => setEditingTask((p) => ({ ...p, draft: v })),
        onTaskCommitEdit, onDeleteTask,
        ...
        onColumnDraftChange, onColumnCommit: () => void commitEditColumn(), onColumnCancel: cancelEditColumn
    });
    ...
    return (
        <div className="min-h-screen bg-zinc-50">
            <ConfirmModal
                ...
                confirmLabel="Xóa trạng thái" cancelLabel="Hủy"
