                ...
                {memberAvatarsToShow.length > 0 && (
                    <div className="mt-2 flex items-center gap-2">
                        {memberAvatarsToShow.map((avatarUrl, idx) => (
                            <div key={avatarUrl || `avatar-${idx}`} className="relative h-7 w-7 overflow-hidden rounded-full ring-2 ring-white">
                                <Image src={avatarUrl} alt={`Member ${idx + 1}`} fill className="object-cover" sizes="28px" />
                            </div>
                        ))}
